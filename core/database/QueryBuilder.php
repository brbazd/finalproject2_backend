...
class QueryBuilder
{
    ...
    public function selectFew($table,$amount,$offset)
    {
        $stmt = $this->pdo->prepare("SELECT * FROM $table ORDER BY id ASC LIMIT $amount OFFSET $offset");
        $stmt->execute();
        return $stmt->fetchAll(PDO::FETCH_OBJ);
    }
    ...
    public function deleteTwo($table,$data1,$data2)
    {
        $stmt = $this->pdo->prepare("DELETE FROM $table WHERE image_id = $data1 AND tag_id = $data2");
        return $stmt->execute();
    }
